 ...
 * to come to an agreement about some arbitrary data (in this case, a [Referral]) encapsulated within a [DealState].
 *
 ...
 */
object ReferralFlow {
    class Initiator(val referral: DealState,
                    val otherParty: Party,
                    override val progressTracker: ProgressTracker = Initiator.tracker()): FlowLogic<ReferralFlowResult>() {
        /**
         * The progress tracker checkpoints each stage of the flow and outputs the specified messages when each
         * checkpoint is reached in the code. See the 'progressTracker.currentStep' expressions within the call() function.
         */
        companion object {
            object CONSTRUCTING_OFFER : ProgressTracker.Step("Constructing proposed referral.")
            object SENDING_OFFER : ProgressTracker.Step("Sending referral to recipient for review.")
            object RECEIVED_PARTIAL_TRANSACTION : ProgressTracker.Step("Received partially signed transaction from recipient.")
         ...
         */
        @Suspendable
        override fun call(): ReferralFlowResult {
            ...
            try {
                ...
                val offerMessage = TransactionState(referral, notary)
                // Stage 2.
                ...
                send(otherParty, ntx)
                return ReferralFlowResult.Success("Transaction id ${ntx.id} committed to ledger.")
            } catch(ex: Exception) {
                // Just catch all exception types.
                return ReferralFlowResult.Failure(ex.message)
            }
        }
    }

    class Acceptor(val otherParty: Party,
                   override val progressTracker: ProgressTracker = Acceptor.tracker()): FlowLogic<ReferralFlowResult>() {
        companion object {
            object WAITING_FOR_PROPOSAL : ProgressTracker.Step("Receiving proposed referral from referer.")
            object GENERATING_TRANSACTION : ProgressTracker.Step("Generating transaction based on proposed referral.")
            object SIGNING : ProgressTracker.Step("Signing proposed transaction with our private key.")
            object SEND_TRANSACTION_AND_WAIT_FOR_RESPONSE : ProgressTracker.Step("Sending partially signed transaction to referer and wait for a response.")
 ...
 */
sealed class ReferralFlowResult {
    class Success(val message: String?): ReferralFlowResult() {
        override fun toString(): String = "Success($message)"
    }

    class Failure(val message: String?): ReferralFlowResult() {
